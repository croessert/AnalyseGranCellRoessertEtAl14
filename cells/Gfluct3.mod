...
MODIFIED BY
Christian Roessert, January 2012 - christian.a@roessert.de
...
BREAKPOINT {
    SOLVE oup
    if(tau_e==0) { g_e = std_e * noise_e }
    if(tau_i==0) { g_i = std_i * noise_i }
    g_e = g_e0 + g_e1
...
PROCEDURE oup() {		: use Scop function normrand(mean, std_dev)
    if(tau_e!=0) { g_e1 =  exp_e * g_e1 + amp_e * noise_e }
    if(tau_i!=0) { g_i1 =  exp_i * g_i1 + amp_i * noise_i }
}
